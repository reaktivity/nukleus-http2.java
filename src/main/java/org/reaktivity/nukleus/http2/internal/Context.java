...
public final class Context implements Closeable
{
        ...
        Configuration config)
    {
        try
        {
            ...
            streamsPath(configDirectory.resolve("http2/streams"));

            sourceStreamsPath(source -> configDirectory.resolve(format("http2/streams/%s", source)));

            targetStreamsPath((source, target) -> configDirectory.resolve(format("%s/streams/http2#%s", target, source)));

            this.controlRO = controlRW.controlPath(config.directory().resolve("http2/control"))
